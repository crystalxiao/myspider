...
from tutorial.settings import KEY_WORD
...
class FiveJobSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://m.51job.com/search/joblist.php?jobarea=030200&keyword={}&pageno=1".format(KEY_WORD),
    ]
    custom_settings = {
    ...
    def parse(self, response):
        res_css = response.css
        items = res_css(".items a").xpath("@href").extract()
        for url in items:
            yield  Request(url,callback=self.parse_item)

        if self.current_page < self.max_page:
            self.current_page += 1
            nex_page = 'https://m.51job.com/search/joblist.php?jobarea=020000&keyword={}&pageno='.format(KEY_WORD)+str(self.current_page)
