...
from jobSpider.settings import UA_LIST, CHANGE_PROXY_COUNTER
import random
import logging
...
class UAMiddleware(object):
    def process_request(self, request, spider):
        ...
        logging.info('****************请求UA为:%s****************' % (request.headers['User-Agent']))

class ProxyMiddlware(object):
    counter = 0
    def process_request(self, request, spider):
        if self.counter < CHANGE_PROXY_COUNTER:
            self.counter += 1
        else:
            self.change_proxy()

    def change_proxy(self):
        status, output = getstatusoutput('adsl-stop;adsl-start')
        if status == 0:
            pass
