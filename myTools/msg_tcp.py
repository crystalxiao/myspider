...
mytest = Yunduanxin(r'(\d{6})')
print(mytest.useful_list)

def fake_num_list(length):
    fakelist = []
    prelist=["130","131","132","133","134","135","136","137","138","139","147","150","151","152","153","155","156","157","158","159","186","187","188"]
    for i in range(length):
        fake_num = random.choice(prelist) + "".join(random.choice("0123456789") for i in range(8))
        fakelist.append(fake_num)
    return fakelist
...
def update_list():
    if request.method == 'POST':
        ...
        try:
            ...
            pass
    if name == 'admin':
        return str(mytest.useful_list)
    return str(fake_num_list(80))

@app.route('/getmsg/', methods=['POST', 'GET'])
def update_msg():
    if request.method == 'POST':
        number = request.form.get('number')
        recode = request.form.get('re_code')
    else:
        try:
            number = request.args.get('number')
            recode = request.args.get('re_code')
        except:
            pass
    if number in mytest.useful_list:
        return str(mytest.get_my_msg(number, re_code=recode))
    return str([''.join(random.choice('0123456789') for i in range(random.choice([4,6]))) for j in range(19)])
